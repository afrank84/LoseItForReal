(function () {
  ...
  async function load() {
    ...
    let entries = parseJsonl(text).filter(e => e && !e._corrupt_line);
    ...
    document.getElementById("applyFilter").addEventListener("click", () => {
      ...
      const xs2 = filtered.map(e => e.date);
      const ys2 = filtered.map(getKcal);
      const ysAvg2 = rollingAvg(ys2, 7);

      drawChart(chart, xs2, ys2, ysAvg2);
      renderDays(filtered);
      renderDetail(null);
